...
int main(int argc, char** argv) {
    const bool file_session = argc == 6 && std::string_view{argv[1]} == "file-session";
    const bool exhaustion = argc == 4 && std::string_view{argv[1]} == "application-exhaustion";
    const bool application = argc == 4 && (std::string_view{argv[1]} == "application" || exhaustion);
    ...
    if (!fixture && !production && !console && !io_test && !io_session && !file_session && !application) {
        std::cerr << "usage: deploypack OUTPUT | deploypack production OUTPUT"
                     ...
                     " | deploypack file-session OUTPUT BLOCK.ELF FILES.ELF CLIENT.ELF"
                     " | deploypack application[-exhaustion] OUTPUT HELLO.ELF\n";
        return 2;
    }
    try {
        const auto bytes = file_session ? myos::deploy::host::pack_file_session(argv + 3)
            : application ? myos::deploy::host::pack_application(argv[3], exhaustion ? 9'000'000 : 1'000'000)
        ...
        if (console || io_test || io_session || file_session || application) {
        ...
        const char* output_path = (production || console || io_test || io_session || file_session || application) ? argv[2] : argv[1];
